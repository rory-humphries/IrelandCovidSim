...
function compute_r0!(u, t, integrator)
    @unpack p = integrator
    @unpack phase_β, μ, α, κ, phase, n, W = p
    ...
    Id = LinearAlgebra.I
    T = -β .* diagm(S ./ N)
    Σ = -((μ + κ + α) .* Id) + W' - diagm(W * ones(size(W, 1)))
    G = -T * inv(Σ)
    ρ = abs(eigs(G; nev=1)[1][1])
    return ρ
end

r0 = SavedValues(Float64, Float64)
cb = SavingCallback(compute_r0!, r0; saveat=[t for t in 0:Tmax])
...
sol = solve(
    prob;
    callback=CallbackSet(update_params_cb, cb),
    progress=true,
    progress_steps=1,
    tstops=tstops,
)
...
sim_df.R0 = r0.saveval

save(project_path("data", "processed", "initial_outbreak_reopen_r0.jld2"), "df", sim_df)
